        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (wtx.IsCoinBase()) {
					...
					if (ExtractDestination(txout.scriptPubKey, type, address) && IsMine(*wallet, address)) {
						bool IsHolding = (strcmp(GetTxnOutputType(type), "holding") == 0);
						if(IsHolding) {
							// Received by ANES Address
							sub.type = TransactionRecord::Holding;
						} else {
							// Received by ANES Address
