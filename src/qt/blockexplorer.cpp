...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::ANES, nValue);
...
{
    std::string genesisblockhash = "000008062c38ddf61a59519ff9ab9aed760d2358b21e802d12dc629b71667e29";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (anes.conf).");
            QMessageBox::warning(this, "ANES Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
