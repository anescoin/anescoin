...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ANESMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ANESMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ANESMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("anes-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ANESMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ANESMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadANESMiner() exception");
    } catch (...) {
        LogPrintf("ThreadANESMiner() exception");
    }

    LogPrintf("ThreadANESMiner exiting\n");
}
