...
{
private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl = NULL, AvailableCoinsType coin_type = ALL_COINS, bool useIX = true, bool forceConf = false) const;
...
public:
    ...
    /// Get 1000ANES output and keys which can be used for the Masternode
    ...
    CAmount GetHoldingBalance() const;
	...
	bool CreateHoldingTransaction(CScript scriptPubKey,
		int nLevel,
        CWalletTx& wtxNew,
        CReserveKey& reservekey,
        CAmount& nFeeRet,
