...
#if defined(NDEBUG)
#error "ANES cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ANES Signed Message:\n";
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
		...
		if (!tx.IsCoinStake()) {
			if (nValueIn < OutValue)
				return state.DoS(100, error("CheckInputs() : %s value in (%s) < value out (%s)",
										  tx.GetHash().ToString(), FormatMoney(nValueIn), FormatMoney(OutValue)),
					REJECT_INVALID, "bad-txns-in-belowout");

			// Tally transaction fees
			CAmount nTxFee = nValueIn - OutValue;
			if (nTxFee < 0)
...
CBitcoinAddress GetAccountAddressForHolding(string strAccount, bool bForceNew = false)
{
    ...
    if (!account.vchPubKey.IsValid() || bForceNew || bKeyUsed) {
        if (!pwalletMain->GetKeyFromPool(account.vchPubKey))
            throw "Error: Keypool ran out, please call keypoolrefill first";
...
void HoldingMoney(const string& strAccount, int nLevel)
{
	...
	int nHoldingWaitBlock = Params().GetHoldingWaitBlock(nLevel);
	if(nHoldingWaitBlock == 0)
		throw "invalid level";
    ...
    CBitcoinAddress address = GetAccountAddressForHolding(strAccount);
    // Parse ANES address
    CScript scriptPubKey = GetScriptForHolding(address.Get(), nHoldingWaitBlock);

    // Create and send the transaction
    CReserveKey reservekey(pwalletMain);
    CAmount nFeeRequired;
	string strError;
    if (!pwalletMain->CreateHoldingTransaction(scriptPubKey, nLevel, wtxNew, reservekey, nFeeRequired, strError, NULL, ALL_COINS, (CAmount)0)) {
