...
CAmount CWallet::GetHoldingBalance() const
{
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, bool forceConf) const
{
    ...
    return (SelectCoinsMinConf(nTargetValue, 1, 6, vCoins, setCoinsRet, nValueRet) ||
            ...
            (!forceConf && bSpendZeroConfChange && SelectCoinsMinConf(nTargetValue, 0, 1, vCoins, setCoinsRet, nValueRet)));
}
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-anes-address
                    CScript scriptChange;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    BOOST_FOREACH (PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins) {
        ...
        if (CheckStakeKernelHash(nBits, block, *pcoin.first, prevoutStake, nTxNewTime, nHashDrift, false, hashProofOfStake, true)) {
            ...
            if (whichType != TX_PUBKEY && whichType != TX_PUBKEYHASH && whichType != TX_HOLDING) {
                if (fDebug && GetBoolArg("-printcoinstake", false))
                    LogPrintf("CreateCoinStake : no support for kernel type=%d\n", whichType);
                break; // only support pay to public key and pay to address
            }
            if (whichType == TX_PUBKEYHASH || whichType == TX_HOLDING) // pay to address type
            {
...
bool CWallet::CreateHoldingTransaction(CScript scriptPubKey,
	int nLevel,
    CWalletTx& wtxNew,
    CReserveKey& reservekey,
    CAmount& nFeeRet,
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
				...
				CTxOut txout(nReturnValue, scriptPubKey);
				if (txout.IsDust(::minRelayTxFee)) {
					strFailReason = _("Transaction amount too small");
					return false;
				}
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, false, true)) {
                    if (coin_type == ALL_COINS) {
                        strFailReason = _("Insufficient confirmed funds.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    if (newTxOut.IsDust(::minRelayTxFee)) {
                        ...
                        // Insert change txn at last position
                        vector<CTxOut>::iterator position = txNew.vout.begin() + txNew.vout.size();
                        txNew.vout.insert(position, newTxOut);
                    }
                } else
                    reservekey.ReturnKey();
